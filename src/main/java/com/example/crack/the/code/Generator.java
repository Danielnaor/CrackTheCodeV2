   ...
   // private Integer maxNumberOfAttempts = 1000; // if the generator fails to generate a clue after 1000 attempts, it will throw an error
    ...
    private Clue generateClue() {
     ...
     if(lengthOfCode != this.correctDigits){
        ...
        if(incorrectDigits <= 0){
            throw new Error("Error: incorrectDigits is less than or equal to 0 - check the Generator class - generateClue method - the par of the code that places the correct digits");
        }
       ...
       for(int i = 0; i < this.correctDigits; i++){
        ...
        if(this.correctDigits > lengthOfCode || correctDigitsIndexes.length > lengthOfCode || correctDigitsList.length > lengthOfCode){
         ...
         if(this.incorrectDigits > 0){ 
            // technically redundant since we already checked that incorrectDigits is greater than 0. 

                        // store the incorrect digits in the incorrectDigitsList
            ...
            for(int i = 0; i < lengthOfCode - this.correctDigits; i++){
                // randomly choose a index from the incorrectNumbers set
                 randomIndex = (int)(Math.random() * incorrectNumbers.size());
                // if not allowing for repeating digits, make sure the digit is not already in the incorrectDigits list

                Integer digit = incorrectNumbers.toArray(new Integer[incorrectNumbers.size()])[randomIndex];

                // combine the while and if values
                while(!repeatDigits && Arrays.asList(incorrectDigitsList).contains(digit)){
                    randomIndex = (int)(Math.random() * incorrectNumbers.size());
                    digit = incorrectNumbers.toArray(new Integer[incorrectNumbers.size()])[randomIndex];
                }
        ...
        // HashSet<Integer> - every time we are putting a digit in the combination (regardless of the placement of the digit and regardless of whether it is correct or incorrect).
        // will be used when reapeating digits are not allowed
        HashSet<Integer> DigitPlaced = new HashSet<Integer>();
        ...
        if(this.correctPlacement > 0){
            for(int i = 0; i < this.correctPlacement; i++){
                // randomly choose a index from the correctDigits list
                 randomIndex = (int)(Math.random() * correctDigitsList.length);
                ...
                while(!repeatDigits && Arrays.asList(combination).contains(correctDigitsList[randomIndex])){
                    randomIndex = (int)(Math.random() * correctDigitsList.length);
                }
        ...
        if(this.incorrectPlacement > 0){
            ...
            if(correctDigitsList.length == 0){
                throw new Error("Error: correctDigitsList is empty - check the Generator class - generateClue method - the par of the code that places the correct digits that are incorrectly placed");
            }
            ...
            for(int i = 0; i < this.incorrectPlacement; i++){
                ...
                // since we are placing the correct digits that are incorrectly placed, we need to make sure that 
                // 1. the index is not the same as the index of the correct digit in the code 
                // 2.  make sure that the index is not already taken
        ...
        if(this.incorrectDigits > 0){
           ...
           // Integer[] indexesNotTaken = new Integer[lengthOfCode - this.correctDigits]; // correctDigits is the number of correct digits regardless of placement and equals to correctPlacement + incorrectPlacement
            ...
            this is is incorrect since getting Exception in thread "main" java.lang.ArrayIndexOutOfBoundsException: Index 3 out of bounds for length 3

            */
            ArrayList<Integer> indexesNotTaken = new ArrayList<Integer>();
            for(int i = 0; i < combination.length; i++){
                if(combination[i] == null){
                    numberOfIndexesNotTaken++;
                    indexesNotTaken.add(i);
                }
            }
            ...
            System.out.println("incorrectDigitsList:" + String.valueOf(incorrectDigitsList.length));


           // put the incorrect digits in the indexes that are not taken 
            for(int i = 0; i < numberOfIndexesNotTaken; i++){
                // use the indexesNotTaken arraylist to get the index that is not taken
                  randomIndex = (int)(Math.random() * indexesNotTaken.size());    

                 
                // place the digit in one of the eampty indexs
                combination[indexesNotTaken.get(randomIndex)] = incorrectDigitsList[i]; 
                System.out.println("i: " + i + " is placed in index: " + indexesNotTaken.get(randomIndex) + " and the digit is: " + incorrectDigitsList[i]);
                

                
    
                // remove the index from the indexesNotTaken array
                indexesNotTaken.remove(randomIndex);
              }
        ...
        if(!validator.checkIfComboValid(combination, clue)){
            String message = "info for debug: " + "\n" 
            + "code is: " + Arrays.toString(code) + "\n"
            + "combination is: " + Arrays.toString(combination) + "\n"
            + "correctDigits: " + correctDigits + " correctPlacement: " + correctPlacement + " incorrectPlacement: " + incorrectPlacement + "\n"
            + incorrectDigits + " incorrectDigits: " 
            // print last debug message
            + validator.getDebugMessages().get(validator.getDebugMessages().size() - 1);
            ...
            System.out.println("combination is valid");
    ...
    // validate the clues
    public boolean validateClues(Riddle riddle){
